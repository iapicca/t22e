...
import 'package:t22e/src/parser/osc_parser.dart';
...
void main() {
  ...
  test('terminal title', () {
    final event = parser.parse(OscSequenceData('0;Hello World'));
  ...
  test('foreground color query response', () {
    final event = parser.parse(OscSequenceData('10;rgb:ff00/8000/4000'));
  ...
  test('background color query response', () {
    final event = parser.parse(OscSequenceData('11;rgb:0000/0000/0000'));
    expect(event, isA<ColorQueryEvent>());
    final colorEvent = event as ColorQueryEvent;
  ...
  test('hyperlink', () {
    final event = parser.parse(OscSequenceData('8;;https://dart.dev'));
    expect(event, isA<InternalEvent>());
  ...
  test('unknown OSC command returns null', () {
    final event = parser.parse(OscSequenceData('99;unknown'));
    expect(event, isNull);
  });

  test('malformed content without semicolon returns null', () {
    final event = parser.parse(OscSequenceData('notvalid'));
    expect(event, isNull);
  });
}
