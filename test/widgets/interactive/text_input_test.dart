...
import 'package:t22e/src/widgets/interactive/text_input.dart';
  ...
  group('TextInput', () {
    ...
    test('inserting a character', () {
      ...
      final (updated, _) = input.update(KeyMsg(
        const KeyEvent(keyCode: KeyCode.char, codepoint: 97), // 'a'
      ));
    ...
    test('inserting multiple characters', () {
      var input = TextInput();
      for (final cp in [97, 98, 99]) { // 'a', 'b', 'c'
        final (updated, _) = input.update(KeyMsg(
    ...
    test('password mode masks value', () {
      ...
      // We verify via view(), not directly testing _displayValue
