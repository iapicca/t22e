...
void main() {
  ...
  group('DiffResult.fromFrames', () {
    ...
    test('content change detects changed row', () {
      final prev = Frame.fromSurface(Surface.genetate(const Size(5, 3)));

      final currSurface = Surface.genetate(const Size(5, 3));
    ...
    test('style-only change is detected', () {
      final prevSurface = Surface.genetate(const Size(5, 3));
    ...
    test('resize larger detects new rows', () {
      final prev = Frame.fromSurface(Surface.genetate(const Size(5, 2)));
      final curr = Frame.fromSurface(Surface.genetate(const Size(5, 5)));
      final result = DiffResult.fromFrames(prev, curr);
      expect(result, [2, 3, 4]);
    });

    test('resize smaller detects removed rows', () {
      final prev = Frame.fromSurface(Surface.genetate(const Size(5, 5)));
      final curr = Frame.fromSurface(Surface.genetate(const Size(5, 2)));
      final result = DiffResult.fromFrames(prev, curr);
      expect(result, [2, 3, 4]);
    });
    ...
    test('multiple changes detected', () {
      final prev = Frame.fromSurface(Surface.genetate(const Size(5, 5)));
      final currSurface = Surface.genetate(const Size(5, 5));
