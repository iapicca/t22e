...
void main() {
  group('ListView', () {
    final items = [ListItem('One'), ListItem('Two'), ListItem('Three')];
    ...
    test('arrow down moves selection', () {
      final list = ListView(items: items);
      final (updated, _) = list.update(
        KeyMsg(const KeyEvent(keyCode: KeyCode.down)),
      );
    ...
    test('arrow up moves selection', () {
      ...
      final (updated, _) = list.update(
        KeyMsg(const KeyEvent(keyCode: KeyCode.up)),
      );
    ...
    test('space toggles multi-select', () {
      ...
      final (updated, _) = list.update(
        KeyMsg(const KeyEvent(keyCode: KeyCode.space)),
      );
    ...
    test('enter does not crash', () {
      final list = ListView(items: items);
      expect(
        () => list.update(KeyMsg(const KeyEvent(keyCode: KeyCode.enter))),
        returnsNormally,
      );
    });
    ...
    test('home goes to first item', () {
      final list = ListView(items: items, selectedIndex: 2);
      final (updated, _) = list.update(
        KeyMsg(const KeyEvent(keyCode: KeyCode.home)),
      );
      expect(updated.selectedIndex, 0);
    });
    ...
    test('end goes to last item', () {
      final list = ListView(items: items, selectedIndex: 0);
      final (updated, _) = list.update(
        KeyMsg(const KeyEvent(keyCode: KeyCode.end)),
      );
      expect(updated.selectedIndex, 2);
    });
  });
