...
void main() {
  test('terminal title', () {
    final event = parseOsc(SequenceData.osc('0;Hello World'));
  ...
  test('foreground color query response', () {
    final event = parseOsc(SequenceData.osc('10;rgb:ff00/8000/4000'));
  ...
  test('background color query response', () {
    final event = parseOsc(SequenceData.osc('11;rgb:0000/0000/0000'));
    expect(event, isA<ColorQueryEvent>());
    final colorEvent = event as ColorQueryEvent;
  ...
  test('hyperlink', () {
    final event = parseOsc(SequenceData.osc('8;;https://dart.dev'));
    expect(event, isA<InternalEvent>());
  ...
  test('clipboard', () {
    final event = parseOsc(SequenceData.osc('52;c;SGVsbG8='));
  ...
  test('unknown OSC command returns null', () {
    final event = parseOsc(SequenceData.osc('99;unknown'));
    expect(event, isNull);
  });

  test('malformed content without semicolon returns null', () {
    final event = parseOsc(SequenceData.osc('notvalid'));
    expect(event, isNull);
  });
}
