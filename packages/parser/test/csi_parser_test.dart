...
void main() {
  ...
  group('home/end', () {
    test('home', () {
      final event = parseCsi(csi(0x48, [1]));
      expect((event as KeyEvent).keyCode, equals(KeyCode.home));
    });

    test('end', () {
      final event = parseCsi(csi(0x46, [1]));
  ...
  group('F-keys via SS3', () {
    test('F1', () {
      final event = parseCsi(csi(0x50));
      expect((event as KeyEvent).keyCode, equals(KeyCode.f1));
    });

    test('F4', () {
      final event = parseCsi(csi(0x53));
  ...
  group('CPR', () {
    test('cursor position report', () {
      final event = parseCsi(csi(0x52, [5, 10]));
