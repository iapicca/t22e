...
void main() {
  group('Surface', () {
    test('constructs grid of correct dimensions', () {
      final s = Surface.genetate(const Size(10, 5));
      expect(s.width, 10);
      expect(s.height, 5);
      expect(s.grid.height, 5);
      expect(s.grid.width, 10);
    ...
    test('putChar places character at position', () {
      final s = Surface.genetate(const Size(5, 3));
    ...
    test('putText places grapheme clusters', () {
      final s = Surface.genetate(const Size(10, 3));
    ...
    test('fillRect fills region', () {
      final s = Surface.genetate(const Size(5, 5));
    ...
    test('drawBorder draws box around region', () {
      final s = Surface.genetate(const Size(10, 8));
    ...
    test('drawBorder with title', () {
      final s = Surface.genetate(const Size(20, 5));
    ...
    test('toPlainLines returns correct strings', () {
      final s = Surface.genetate(const Size(4, 3));
    ...
    test('resize preserves overlapping region', () {
      final s = Surface.genetate(const Size(10, 10));
    ...
    test('resize smaller clips content', () {
      ...
      expect(resized.grid.height, 5);
      expect(resized.grid.width, 5);
    });
    ...
    test('putText applies style to all chars', () {
      ...
      expect(
        s.grid[1][1].style.foreground,
        const Color(red: 0, green: 153, blue: 0),
      );
      expect(
        s.grid[1][2].style.foreground,
        const Color(red: 0, green: 153, blue: 0),
      );
    });
